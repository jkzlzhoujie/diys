...
@Component
@Repository("circlePostDiscDao")
public class CirclePostDiscDaoImpl extends SimpleMybatisSupport<CirclePostDisc, Long> implements CirclePostDiscDao{

	@Override
	public String getMybatisMapperNamesapce() {
		return "cn.temobi.complex.entity.CirclePostDisc";
	}
	...
	public Page findByPageTwo(Page page, Map<String, String> parameter) {
		...
		if(map ==null || map.size() ==0){//没有参数时 ，直接查总表数量 
			...
			Long tempCount = (Long) CacheHelper.getInstance().get("circlePostDisc@checkCount");
			if(tempCount == null){
				totalItems = (Number) getSqlSession().selectOne(toMybatisStatement("count"), toParameterMap(parameter));
				CacheHelper.getInstance().set(60*60*24,"circlePostDisc@checkCount", totalItems);
			}else{
