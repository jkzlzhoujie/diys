...
@Component
@Repository("cmCircleUserDao")
public class CmCircleUserDaoImpl extends SimpleMybatisSupport<CmCircleUser, Long> implements CmCircleUserDao{

	@Override
	public String getMybatisMapperNamesapce() {
		return "cn.temobi.complex.entity.CmCircleUser";
	}
	
	@Override
	public Page<CmUserInfoListDto> findUser(
			Page<CmUserInfoListDto> page, Object parameter) {
		Number totalItems = (Number) getSqlSession().selectOne(toMybatisStatement("countUser"), toParameterMap(parameter));
        if (totalItems != null && totalItems.longValue() > 0) {
            List list = getSqlSession().selectList(toMybatisStatement("findUser"), toParameterMap(parameter, page));
            page.setResult(list);
	...
	public Page findByPageTwo(Page page, Map<String, String> parameter) {
		...
		if(map ==null || map.size() ==0){//没有参数时 ，直接查总表数量 
			...
			Long tempCount = (Long) CacheHelper.getInstance().get("circleUser@checkCount");
			if(tempCount == null){
				totalItems = (Number) getSqlSession().selectOne(toMybatisStatement("count"), toParameterMap(parameter));
				CacheHelper.getInstance().set(60*60*24,"circleUser@checkCount", totalItems);
			}else{
